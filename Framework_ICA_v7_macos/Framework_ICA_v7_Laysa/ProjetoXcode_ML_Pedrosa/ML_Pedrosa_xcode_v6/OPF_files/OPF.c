...
char   opf_PrecomputedDistance = 0;
float  **opf_DistanceValue;

opf_ArcWeightFun opf_ArcWeight = opf_EuclDist;//opf_EuclDistLog;

float gammaGaussDist = -1;
...
int setDistanceOPFPedrosa(int distance)
{
    //1 - Euclidean
    //2 - Chi-Square
    //3 - Manhattan (L1)
    ...
    switch (distance) {
        ...
        case 2:
            opf_ArcWeight = opf_ChiSquaredDist;
            break;
            
        case 3:
            opf_ArcWeight = opf_ManhattanDist;
            break;
            
        case 4:
            opf_ArcWeight = opf_CanberraDist;
            break;
            
        case 5:
            opf_ArcWeight = opf_SquaredChordDist;
            break;
            
        case 6:
            opf_ArcWeight = opf_SquaredChiSquaredDist;
            break;
            
        case 7:
            opf_ArcWeight = opf_BrayCurtisDist;
            break;
            
        case 8:
            opf_ArcWeight = opf_GaussDist;
            break;
            
        case 9:
            opf_ArcWeight = opf_MahanlabobisDist;
            break;
...
// Compute gaussian distance between feature vectors  // 8 - GaussDist
void setGammaGaussDist(float gamma)
{
    gammaGaussDist = gamma;
    
}

float opf_GaussDist(float *f1, float *f2, int n)
{
    ...
    dist = 1- exp(-(dist/(2*gammaGaussDist*gammaGaussDist)));
    
    return(dist);
    
}

float covarianceF(float *A, float meanA, int length)
// Function calculates the sample covariance the array A and array B, whose lengths are "length"
{
    double SOS = 0;
    for (int i=0; i<length; i++)
    {  SOS += (A[i] - meanA) * (A[i] - meanA); }
    return SOS / length;
}

float opf_MahanlabobisDist(float *f1, float *f2, int n)
{
    ...
    float mean2=0;
    float a,b;
    float aux1,auxx1,auxx2,pC,temp1,temp2,pcInv,d,aux2=0;
    ...
    diff = mean1-mean2;
    cA = covarianceF(f1,n,mean1);
    cB = covarianceF(f2,n,mean2);
    
    auxx1 = (float)n1/(float)nn;
    auxx2 = (float)n2/(float)nn;
    aux1 = auxx1*cA;
    aux2 = auxx2*cB;
